...
class BillingWindow(tk.Toplevel):
    def __init__(self, master: tk.Misc):
        ...
        self.title("Billing and invoices")
    ...
    def _build_ui(self) -> None:
        ...
        header = build_header(container, "Billing", "Manage invoices and payments")
        ...
        form = ttk.LabelFrame(main, text="Invoice details", padding=14)
        form.grid(row=0, column=0, sticky="nsew", padx=(0, 10))

        ttk.Label(form, text="Patient name (for lookup)").grid(row=0, column=0, sticky="w", pady=4)
        ttk.Entry(form, textvariable=self.patient_name_var).grid(row=0, column=1, sticky="ew", pady=4)

        ttk.Label(form, text="Total amount").grid(row=1, column=0, sticky="w", pady=4)
        ttk.Entry(form, textvariable=self.amount_var).grid(row=1, column=1, sticky="ew", pady=4)
        ...
        self.status_combo.grid(row=2, column=1, sticky="ew", pady=4)

        ttk.Label(form, text="Notes").grid(row=3, column=0, sticky="w", pady=4)
        ttk.Entry(form, textvariable=self.notes_var).grid(row=3, column=1, sticky="ew", pady=4)

        form.columnconfigure(1, weight=1)

        button_row = ttk.Frame(form)
        button_row.grid(row=4, column=0, columnspan=2, pady=(12, 0))
        ...
        ttk.Label(search_row, text="Search (patient / status):").pack(side="left")
        ...
        columns = ("id", "patient", "created_at", "amount", "status", "notes")
    ...
    def _load_rows(self, search: str | None = None) -> None:
        ...
        with get_connection() as conn:
            cur = conn.cursor()
            sql = (
                "SELECT i.id, p.full_name, i.created_at, i.total_amount, i.status, COALESCE(i.notes,'') "
                "FROM invoice i JOIN patient p ON i.patient_id = p.id"
            )
            params = ()
            if search:
                like = f"%{search.strip()}%"
                sql += " WHERE p.full_name LIKE ? OR i.status LIKE ?"
                params = (like, like)
    ...
    def _on_save(self) -> None:
        ...
        try:
            with get_connection() as conn:
                ...
                if self.selected_id is None:
                    ...
                    cur.execute(
                        """
                        UPDATE invoice
                        SET patient_id=?, total_amount=?, status=?, notes=?
                        WHERE id=?
                        """,
            ...
            messagebox.showerror("Database error", f"Could not save invoice: {exc}")
            return
    ...
    def _on_delete(self) -> None:
        ...
        try:
            with get_connection() as conn:
                cur = conn.cursor()
                cur.execute("DELETE FROM invoice WHERE id=?", (self.selected_id,))
                conn.commit()
        except Exception as exc:
    ...
    def _on_select(self, event=None) -> None:
        ...
        if not values:
            return
        iid, patient, created, amount, status, notes = values
